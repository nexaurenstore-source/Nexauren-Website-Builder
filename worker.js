...
const textAsset=(body,type='text/plain; charset=utf-8')=>new Response(body,{headers:{'content-type':type}});

// Files are concatenated by the build step below for Cloudflare Workers.
...
async function handleRequest(request,env){
  ...
  if(url.pathname==='/api/billing/payment'&&request.method==='GET'){const u=await billingCurrentUser(request,env);if(!u)return json({error:'Authentication required.'},401,cors(request));const ref=String(url.searchParams.get('reference')||'');const p=await billingPaymentStatus(env,u.id,ref);return p?json({payment:p},200,cors(request)):json({error:'Payment not found.'},404,cors(request));}
