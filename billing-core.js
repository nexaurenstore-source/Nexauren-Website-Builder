...
async function billingEnsureAccount(env,userId) {
  ...
  const free=await env.DB.prepare("SELECT id,credits_per_cycle FROM plans WHERE id='free' AND enabled=1 LIMIT 1").first();
...
async function billingCreateCheckout(env,request,user,type,productId) {
  ...
  const table=type==='credit_purchase'?'credit_packages':'plans';
  const product=await env.DB.prepare(`SELECT * FROM ${table} WHERE id=?1 AND enabled=1 LIMIT 1`).bind(productId).first();
  ...
  const provider=providerName==='paypal'?createPayPalProvider():null;
  if(!provider) throw new Error('Payment provider not configured.');
  const reference=`order:${billingUuid()}`;
  const paymentId=billingUuid();
  const now=Math.floor(Date.now()/1000);
  ...
  try {
    const checkout=await provider.createCheckout({env,request,user,reference,product,productType:type});
    const providerId=billingClean(checkout?.transaction_id||checkout?.order_id||checkout?.subscription_id);
    await env.DB.prepare("UPDATE payments SET provider_transaction_id=?1,metadata=?2,updated_at=?3 WHERE id=?4 AND status='pending'").bind(providerId||null,JSON.stringify({product_id:productId,checkout_mode:checkout?.mode||null}),Math.floor(Date.now()/1000),paymentId).run();
...
async function billingAddCredits(env,{userId,amount,type,description,reference,paymentId=null,toolId=null}) {
  const credits=Math.floor(Number(amount)); if(!Number.isFinite(credits)||credits<=0) throw new Error('Invalid credit amount.');
  const now=Math.floor(Date.now()/1000);
  const result=await env.DB.batch([
    env.DB.prepare('INSERT OR IGNORE INTO credit_transactions(id,user_id,amount,type,description,reference,payment_id,tool_id,created_at) VALUES(?1,?2,?3,?4,?5,?6,?7,?8,?9)').bind(billingUuid(),userId,credits,type,billingClean(description).slice(0,240),reference,paymentId,toolId,now),
    env.DB.prepare('INSERT OR IGNORE INTO credit_balances(user_id,balance,updated_at) VALUES(?1,0,?2)').bind(userId,now),
    env.DB.prepare('UPDATE credit_balances SET balance=(SELECT COALESCE(SUM(amount),0) FROM credit_transactions WHERE user_id=?1),updated_at=?2 WHERE user_id=?1').bind(userId,now)
  ]);
...
async function billingFinalizePayment(env,verified) {
  ...
  if(type==='credit_purchase') {
    const existing=await env.DB.prepare('SELECT id FROM credit_transactions WHERE reference=?1 LIMIT 1').bind(`payment:${reference}`).first();
    if(existing) return {processed:false,idempotent:true,credit_transaction_id:existing.id};
    const product=await env.DB.prepare('SELECT credits FROM credit_packages WHERE id=?1 AND enabled=1 LIMIT 1').bind(productId).first();
    if(!product) throw new Error('Credit product not found.');
    const result=await billingAddCredits(env,{userId,amount:Number(product.credits),type:'purchase',description:`Credit purchase: ${productId}`,reference:`payment:${reference}`,paymentId:payment.id});
...
async function billingHandle(request,env) {
  const url=new URL(request.url); const path=url.pathname;
  if(path==='/api/billing/catalog'&&request.method==='GET') return billingJson(await billingCatalog(env),200,corsHeaders(request));
  const user=await billingCurrentUser(request,env);
  if(path.startsWith('/api/billing/')&&!user) return billingJson({error:'Authentication required.'},401,corsHeaders(request));
  if(path==='/api/billing/account'&&request.method==='GET') return billingJson(await billingAccount(env,user.id),200,corsHeaders(request));
  if(path==='/api/billing/payment'&&request.method==='GET') { const reference=billingClean(url.searchParams.get('reference')); if(!reference) return billingJson({error:'reference is required.'},400,corsHeaders(request)); const payment=await billingPaymentStatus(env,user.id,reference); return payment?billingJson({payment},200,corsHeaders(request)):billingJson({error:'Payment not found.'},404,corsHeaders(request)); }
  if(path==='/api/billing/checkout'&&request.method==='POST') { const d=await request.json().catch(()=>null); try{return billingJson(await billingCreateCheckout(env,request,user,billingClean(d?.type),billingClean(d?.product_id)),201,corsHeaders(request));}catch(e){console.error('Builder billing checkout',String(e));return billingJson({error:'Unable to create checkout.',detail:String(e.message||e).slice(0,240)},502,corsHeaders(request));} }
  if(path==='/api/billing/credits'&&request.method==='GET') { const account=await billingEnsureAccount(env,user.id); return billingJson({balance:Number(account?.balance||0),plan_id:account?.plan_id||'free'},200,corsHeaders(request)); }
  if(path==='/api/billing/transactions'&&request.method==='GET') { const rows=await env.DB.prepare('SELECT id,amount,type,description,reference,payment_id,tool_id,created_at FROM credit_transactions WHERE user_id=?1 ORDER BY created_at DESC LIMIT 50').bind(user.id).all(); return billingJson({transactions:rows.results||[]},200,corsHeaders(request)); }
  if(path==='/api/billing/subscription/cancel'&&request.method==='POST') { const sub=await env.DB.prepare("SELECT id,provider,provider_subscription_id,status,next_billing_date FROM subscriptions WHERE user_id=?1 AND status IN ('active','past_due') ORDER BY created_at DESC LIMIT 1").bind(user.id).first(); if(!sub)return billingJson({error:'No active subscription.'},404,corsHeaders(request)); try{await createPayPalProvider().subscriptionAction({env,subscriptionId:sub.provider_subscription_id,action:'cancel'});}catch(e){return billingJson({error:'Unable to cancel subscription.'},502,corsHeaders(request));} const now=Math.floor(Date.now()/1000); await env.DB.prepare('UPDATE subscriptions SET cancel_at_period_end=1,updated_at=?1 WHERE id=?2').bind(now,sub.id).run(); return billingJson({success:true,cancel_at_period_end:true,current_period_end:sub.next_billing_date||null},200,corsHeaders(request)); }
  return null;
}

function corsHeaders(request){return {'access-control-allow-origin':request.headers.get('Origin')||'*','access-control-allow-credentials':'true','access-control-allow-headers':'Content-Type, Accept, Authorization','access-control-allow-methods':'GET,POST,OPTIONS'};}
